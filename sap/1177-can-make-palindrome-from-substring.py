...
for replacement so if for example s[left..right] = "aaa", and k = 2, 
we can only replace two of the letters.  
...
class Solution:
    def canMakePaliQueries(self, s: str, queries: List[List[int]]) -> List[bool]:
        return [self.isPalindrome(s[left:right+1], k) for left, right, k in queries]

    def isPalindrome(self, s, k):
        if k > 13:
            return True
