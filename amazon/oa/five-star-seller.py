...
It seems a lot of the links for Amazon OA are broken for some reason. Here's the description for 5 star sellers. Anyone has a good solution for this question?
...
def getRating(productRatings):
    return sum([i[0]/i[1] for i in productRatings]) / len(productRatings)


def fiveStarReviews(productRatings, ratingThreshold):
    cnt = 0
    tempList = productRatings.copy()
    while getRating(tempList) < ratingThreshold / 100:
        distList = [(productRating[0] + 1) / (productRating[1] + 1) - (productRating[0] / productRating[1])
