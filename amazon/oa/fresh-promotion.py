...
purchasing a secret combination of fruits. 
The combination will change each day, and the team running the promotion wants 
to use a code list to make it easy to change the combination. The code list contains groups of fruits. 
...
The customer is a winner as the customer has added fruits in the order of fruits 
in the groups and the order of groups in the codeList is also maintained in the shoppingCart.
...
Explanation:
The customer is not a winner as the first 2 fruits form group 1, all three fruits would form group 2, but can't because it would contain all fruits of group 
...
def isWinner(codeList, shoppingCart):
    ...
    if not shoppingCart:
        return 0
    i, j = 0, 0
    # i : index of the groups
    # j : index of the fruits in the group
    for k in range(len(shoppingCart)):
        # if the fruit in the shopping cart matches the fruit in the code list,
        # check if it is the last fruit of the code list
        # if yes, return trut, else continue the process
        if codeList[i][j] == shoppingCart[k] or codeList[i][j] == 'anything':
            ...
            # if it does not match, reset the index of fruite in the code list to the beginning
            # and skip the current fruit in the shopping cart.
            # if codeList[i][0] == 'anything':
            #    j = 1
            # else:
            j = 0
            # skip k
